...
export function AfternoonRostersPage() {
  ...
  const afternoon = (data ?? []).filter((c) => c.kind === 'AFTERNOON');
  const filtered = afternoon.filter((c) =>
    ...
    <div className="max-w-4xl mx-auto px-4 py-6 space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center gap-3">
        <h1 className="text-2xl font-bold text-text-primary flex-1">
          Afternoon Class Rosters
        </h1>
        <Badge variant="brand">{afternoon.length} classes</Badge>
      </div>
      ...
      {!isLoading && !isError && filtered.length === 0 && (
        <Card>
          <p className="text-text-secondary text-sm text-center py-8">
            {search ? 'No classes match your search.' : 'No afternoon classes found.'}
          </p>
        </Card>
      )}
