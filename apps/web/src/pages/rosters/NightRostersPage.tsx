...
export function NightRostersPage() {
  ...
  const night = (data ?? []).filter((c) => c.kind === 'NIGHT');
  const filtered = night.filter((c) =>
    ...
    <div className="max-w-4xl mx-auto px-4 py-6 space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center gap-3">
        <h1 className="text-2xl font-bold text-text-primary flex-1">
          Night Class / Activity Rosters
        </h1>
        <Badge variant="brand">{night.length} classes</Badge>
      </div>
      ...
      {!isLoading && !isError && filtered.length === 0 && (
        <Card>
          <p className="text-text-secondary text-sm text-center py-8">
            {search ? 'No classes match your search.' : 'No night classes or activities found.'}
          </p>
        </Card>
      )}
